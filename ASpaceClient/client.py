...
class ASpaceClient(object):
    ...
    def _add_instance(self, config):
        ...
        frontend_url = input("Frontend URL: ")
    ...
    def list_archival_objects(self):
        uri = self.repository + "/archival_objects"
        params = {"all_ids": True}
        return self.get_aspace_json(uri, params=params)
    ...
    def get_export_metadata(self, resource_number):
        uri = self.repository + \
            "/resource_descriptions/{}.xml/metadata".format(
                resource_number)
        return self.get_aspace_json(uri)
    ...
    def export_ead(self, resource_number, include_unpublished=False, include_daos=True, numbered_cs=True):
        uri = self.backend_url + self.repository + \
            "/resource_descriptions/" + "{}.xml".format(resource_number)
        params = {
